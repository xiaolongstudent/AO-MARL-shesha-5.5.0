...
def get_breakdown(filename, plot=False):
    """
    ...
    if plot:
        out_folder_bandwith = "h5_files/outputs/bd_" + filename[9:-3] + ".png"
        plt.figure()
        B = np.var(P.dot(bp), axis=1)
        print("B shape", B.shape)
        Bp_list = []
        for mode in range(B.shape[0]):
            current_mode_error = np.sqrt(B[mode]) * 1e3
            Bp_list.append(current_mode_error)
            # print("mode :", mode, "bp", current_mode_error, " nm rms")

        plt.bar(np.arange(len(Bp_list)), Bp_list)
        plt.xlabel("Mode num")
        plt.ylabel(r"$\sigma_\beta$ (nm)")
        plt.ylim(bottom=0, top=20)
        plt.savefig(out_folder_bandwith)
        plt.close()

    # 2pi over lambda anb make it exponential
    ...
    sr_from_error_breakdown_no_RL = np.exp(-(C.sum() * (2 * np.pi / Lambda) ** 2) - f['fitting'][()])
    cov = f["cov"][()]

    psf_compass = get_tar_image(filename)
    print("L.E. SR from error breakdown without RL", sr_from_error_breakdown_no_RL)
    if cov.shape[0] > 6:
        plt.imshow(cov)
        plt.show()
        sr_from_cov = np.exp(-(cov.sum()) * (2 * np.pi / Lambda) ** 2 - f["fitting"][()])
        sr_from_cov_lin = np.exp(-(cov[:6,:6].sum()) * (2 * np.pi / Lambda) ** 2 - f["fitting"][()])
        print("L.E. SR from Cov Mat with RL", sr_from_cov)
        print("L.E. SR from Cov Mat without RL", sr_from_cov_lin)
        print("L.E. SR from Compass", psf_compass.max())
    else:
        sr_from_cov = np.exp(-(cov.sum()) * (2 * np.pi / Lambda) ** 2 - f["fitting"][()])
        print("L.E. SR from Cov Mat Linear", sr_from_cov)
        print("L.E. SR from Compass", psf_compass.max())
    ...
    print("fitting :",
          np.mean(np.sqrt(f["fitting"][()] / ((2 * np.pi / Lambda)**2)) * 1e3),
          " nm rms")
...
def plotCovCor(filename, maparico=None, plot=False, include_anisoplanatism=True, include_noise = True):
    """
    ...
    if not include_anisoplanatism and not include_noise:
        if cov.shape[1] == 6:
            cov = cov[1:-1, 1:-1]
            cor = cor[1:-1, 1:-1]
            labels = ["WF deviation", "aliasing", "filt. modes", "bandwidth"]
        elif cov.shape[1] == 7:
            cov = np.delete(np.delete(cov, -2, 0), -2, 1)
            cor = np.delete(np.delete(cor, -2, 0), -2, 1)
            cov = cov[1:, 1:]
            cor = cor[1:, 1:]
            print("Cov, cor shapes", cov.shape, cor.shape)
            labels = ["WF deviation", "aliasing", "filt. modes", "bandwidth", "RL"]
            for row in range(cov.shape[0]):
                print("Priting diagonal terms of covariance matrix", row, labels[row], cov[row, row])
        else:
            raise NotImplementedError

    elif not include_anisoplanatism:
        if cov.shape[1] == 6:
            cov = cov[:-1, :-1]
            cor = cor[:-1, :-1]
            labels = ["noise", "WF deviation", "aliasing", "filt. modes", "bandwidth"]
        elif cov.shape[1] == 7:
            cov = np.delete(np.delete(cov, -2, 0), -2, 1)
            cor = np.delete(np.delete(cor, -2, 0), -2, 1)
            print("Cov, cor shapes", cov.shape, cor.shape)
            labels = ["noise", "WF deviation", "aliasing", "filt. modes", "bandwidth", "RL"]
            for row in range(cov.shape[0]):
                print("Priting diagonal terms of covariance matrix", row, labels[row], cov[row, row])
        else:
            raise NotImplementedError
        ...
        if cov.shape[1] == 6:
            ...
            labels = ["noise", "WF deviation", "aliasing", "filt. modes", "bandwidth", "aniso", "RL"]
        else:
            raise NotImplementedError

    if (maparico is None):
        maparico = "viridis"
    plt.figure(num=20, figsize=(8, 8))
    x = np.arange(len(labels))
    plt.matshow(cov, cmap=maparico, fignum=20)
    plt.colorbar()
    ...
    plt.figure(num=21, figsize=(8, 8))
    plt.matshow(cor, cmap=maparico, fignum=21)
    plt.colorbar()
    plt.xticks(x, labels, rotation="vertical")
    plt.yticks(x, labels)
    if plot:
