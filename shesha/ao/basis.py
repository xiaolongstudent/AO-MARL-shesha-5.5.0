...
def compute_KL2V(p_controller: conf.Param_controller, dms: Dms, p_dms: list,
    ...
    # TODO changed by Tomeu as I changed the input
    ...
    for i in range(len(p_dms)):
        ...
        if (p_dms[ndm].type == scons.DmType.PZT):
            ...
            dms[ndm].compute_KLbasis(p_dms[ndm]._xpos, p_dms[ndm]._ypos,
                                          indx_valid, indx_valid.size, norm, 1.0)

            KL2V[indx_act:indx_act + ntotact[ndm], indx_act:indx_act + ntotact[ndm]] = \
                np.fliplr(dms[ndm].d_KLbasis)
...
def compute_IFsparse(g_dm: list, p_dms: list, p_geom: conf.Param_geom):
    ...
    for i in range(ndm):
        ...
        IFi = compute_dm_basis(g_dm[i], p_dms[i], p_geom)
        if (i == 0):
            val = IFi.data
            col = IFi.indices
            row = IFi.indptr
