...
from src.reinforcement_learning.rpc_training.efficient_kan.kan import KAN

# LOG_SIG_MAX = 2
LOG_SIG_MIN = -20
epsilon = 1e-5
devices = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
def KAN_weights_init_(m, gain=1):
    if isinstance(m, KAN):
        torch.nn.init.kaiming_uniform_(m.weight, a=math.sqrt(5) * self.scale_base)

        # torch.nn.init.xavier_uniform_(m.weight, gain=gain)
        if hasattr(m, 'bias') and m.bias is not None:
            torch.nn.init.kaiming_uniform_(m.bias ,a=math.sqrt(5) * self.scale_base)
...
class Kan_QNetwork(nn.Module):
    ...
    def __init__(self, num_inputs, num_actions, hidden_dim, num_layers):
        super(Kan_QNetwork, self).__init__()
        self.Q1_input = nn.Linear(num_inputs + num_actions, hidden_dim)
        self.Q1_output = KAN([hidden_dim,1],grid_size = 5)

        self.Q2_input = nn.Linear(num_inputs + num_actions, hidden_dim)

        self.Q2_output = KAN([hidden_dim,1],grid_size = 5)
        # self.Q1_output =KANLayer(hidden_dim, 1, num=5)

        # self.Q2_output =KANLayer(hidden_dim, 1, num=5)
...
class KANGaussianPolicy(nn.Module):
    ...
    def __init__(self, **kwargs):
        super(KANGaussianPolicy, self).__init__()

        # self.mean_linear = KAN([kwargs['num_inputs'], kwargs['hidden_dim'],kwargs['hidden_dim']])
        # self.hidden = nn.ModuleList()
        # for i in range(kwargs['num_layers'] - 1):
        #     self.hidden.append(KANLayer(kwargs['hidden_dim'], kwargs['hidden_dim']))
        ...
        self.mean_linear = KAN([kwargs['hidden_dim'], kwargs['num_actions']])
        self.log_std_linear =KAN([kwargs['hidden_dim'], kwargs['num_actions']])
        ...
        # self.mean_linear = KAN([kwargs['num_inputs'], kwargs['hidden_dim'],kwargs['hidden_dim'],kwargs['num_actions']],grid_size=5)
        #
        # self.log_std_linear = KAN([kwargs['num_inputs'], kwargs['hidden_dim'],kwargs['hidden_dim'],kwargs['num_actions']],grid_size=5)
        #
        # if kwargs['activation'] == "relu":
        #     self.activation = F.relu
        # elif kwargs['activation'] == "leaky_relu":
    ...
    def to(self, device):
        ...
        return super(KANGaussianPolicy, self).to(device)
